...
var Exchange = function() {
	...
	this.on('clean', function(client) {
		...
		for(var i=0; i<clients[client].rooms.length; i++) {
			console.log('removing client from room:', rooms[clients[client].rooms[i]].getId());
			rooms[clients[client].rooms[i]].removeClient(client);
			if(rooms[clients[client].rooms[i]].getClients().length === 0) {
